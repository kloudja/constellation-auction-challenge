...
public class VehicleServiceTests
{
    ...
    public async Task CreateVehicle_AllowedTypes_Success()
    {
        ...
        var createdVehicle = await vehicleService.CreateAsync(createVehicleRequest);
        ...
        var vehiclesInUS = await vehicleService.ListByRegionAsync("US");
        vehiclesInUS.Should().ContainSingle(v => v.Id == createdVehicle.Id);

        var vehiclesInEU = await vehicleService.ListByRegionAsync("EU");
    ...
    public async Task UpdateVehicle_Success()
    {
        ...
        var created = await vehicleService.CreateAsync(new CreateVehicleRequest("EU", "Hatchback", "VW", "Polo", 2019));
        UpdateVehicleRequest updateRequest = new(created.Id, "VW", "Polo GTI", 2021);

        var updated = await vehicleService.UpdateAsync(updateRequest);
        ...
        var reloaded = await vehicleService.GetAsync(created.Id);
    ...
    public async Task SoftDeleteVehicle_HidesFromQueries()
    {
        ...
        var created = await vehicleService.CreateAsync(new CreateVehicleRequest("US", "Truck", "Ford", "F-150", 2020));
